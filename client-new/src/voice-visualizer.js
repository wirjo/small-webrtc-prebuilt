...
class VoiceVisualizer {
    ...
    getFrequencyBinIndex(frequency) {
        ...
        return Math.round((frequency / nyquist) * (this.analyser.frequencyBinCount - 1));
    }
    // In the startVisualization method
    startVisualization() {
        if (!this.canvasCtx || !this.analyser || !this.canvas)
            return;
        const frequencyData = new Uint8Array(this.analyser.frequencyBinCount);
        const scaleFactor = 2;
        const { barWidth, barGap, barMaxHeight, backgroundColor, barColor } = this.options;
        const drawSpectrum = () => {
            // Recheck all variables since they could become null between animation frames
            if (!this.analyser || !this.canvasCtx || !this.canvas)
                return;
            this.analyser.getByteFrequencyData(frequencyData);
            this.canvasCtx.clearRect(0, 0, this.canvas.width / scaleFactor, this.canvas.height / scaleFactor);
            this.canvasCtx.fillStyle = backgroundColor;
            this.canvasCtx.fillRect(0, 0, this.canvas.width / scaleFactor, this.canvas.height / scaleFactor);
            let isActive = false;
            const totalBarsWidth = this.bands.length * barWidth + (this.bands.length - 1) * barGap;
            ...
            this.bands.forEach((band, i) => {
                ...
                const yTop = Math.max(canvasHeight / scaleFactor / 2 - barHeight / 2, adjustedCircleRadius);
                const yBottom = Math.min(canvasHeight / scaleFactor / 2 + barHeight / 2, canvasHeight / scaleFactor - adjustedCircleRadius);
                if (band.smoothValue > 0) {
                    ...
                    ctx.beginPath();
                    ctx.arc(x + barWidth / 2, canvasHeight / scaleFactor / 2, adjustedCircleRadius, 0, 2 * Math.PI);
